...
import { calculateBestOffGrowth, growthKeys } from '../utils/growth';
import { growthEvidence } from '../data/growth-evidence';
...
export function cardMatches(id: string, route: RouteId, note: RosterNote, kind: Category, query: string, mergeOnly: boolean, spoilers: SpoilerData | null = null) {
  const c = characterById[id], guide = guideFor(id, route);
  const text = [c.nameZh, c.nameEn, formatRequirement(recruitment[id]?.[route]), note.role, note.reason, guide.goal ?? note.goal, ...note.tags, formatBuild(guide.paths, spoilers?.classes), guide.reason, categoryLabels[kind]].join(' ').toLowerCase();
  return text.includes(query.trim().toLowerCase()) && (!mergeOnly || kind === 'merge' || note.tags.some(t => t.includes('合流')) || guide.tag.includes('合流'));
}
export function CharacterCard({ id, route, kind, note, checked, onCheck, spoilers }: { id: string; route: RouteId; kind: Category | 'replacement'; note: RosterNote; checked?: boolean; onCheck?: (checked: boolean) => void; spoilers?: SpoilerData | null }) {
  ...
  return <article className={`unit roster-item kind-${kind === 'replacement' ? 'primary' : kind}`} data-character={id}>
    ...
    <BuildPath guide={guide} unlockedClasses={spoilers?.classes} />
    <div className="personal-growth" aria-label="九项个人成长率"><span className="personal-growth-title">个人成长率</span>{growthKeys.map(key => <div key={key}><span>{key.toUpperCase()}</span><b>{growths[id][key]}%</b></div>)}</div>
    <details className="card-details"><summary>个人基础成长率、培养理由{alternatives.length > 0 ? '与备选方案' : ''}</summary><div className="inside">
      <p className="growth-basis">个人成长率沿用现有玩家热力图转录值，职业修正另计；特殊个人特技的统计口径见资料差异。主攻成长合计（去掉力量、魔力中较低一项）：{calculateBestOffGrowth(growths[id])}%。</p>
      {growthEvidence[id] && <p className="growth-evidence">资料差异：{growthEvidence[id]}</p>}
      {!guide.goal && note.goal && <p>培养目标：{note.goal}</p>}
      <p className="reason">{note.reason}</p><p className="class-why">{guide.reason}</p>
      {alternatives.map((path, index) => <div className="alternative-build" key={index}><b>{path.purpose === 'rng-fix' ? 'RNG 救火 · 按条件使用' : path.purpose === 'experimental' ? '实验方案' : '条件 / 备选方案'}</b><p>{formatBuild([path], spoilers?.classes)}</p></div>)}
